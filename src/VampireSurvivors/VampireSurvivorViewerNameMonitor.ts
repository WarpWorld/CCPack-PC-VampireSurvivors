import { getGame, getIsGamePaused, getViewerEnemyGroups, pruneViewerEnemyGroups } from './VampireSurvivorsState'
import { TEXT_COLORS } from './VampireSurvivorsTextColors'

export class VampireSurvivorsViewerNameMonitor {
  ...
  protected onTick = () => {
    ...
    for (const group of viewerEnemyGroups) {
      ...
      group.children?.entries?.forEach?.((enemy) => {
        if (enemy.$viewer) {
          if (!group.active || !group.enabled || !enemy.visible || enemy.isDead) {
            enemy.$viewer.destroy()
            delete enemy.$viewer
          } else {
